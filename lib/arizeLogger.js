...
class ArizeLogger {
   ...
   * @param {Object} data - Voice biomarker data
   */
  logVoiceBiomarkers(data) {
    const timestamp = Date.now();

    const metric = {
      type: 'voice_biomarkers',
      timestamp,
      data: {
        sessionId: data.sessionId,
        jitter: data.biomarkers?.jitter,
        shimmer: data.biomarkers?.shimmer,
        hnr: data.biomarkers?.hnr,
        syllableRate: data.biomarkers?.syllableRate,
        pitchRange: data.biomarkers?.pitchRange,
        loudnessVariation: data.biomarkers?.loudnessVariation,
        speechRate: data.biomarkers?.speechRate,
        pauseRatio: data.biomarkers?.pauseRatio,
        updrsScore: data.updrsScore?.score,
        updrsLabel: data.updrsScore?.label,
        tasksCompleted: data.rawMetrics?.tasksCompleted || 4
      }
    };
    ...
    // Log with OpenTelemetry
    if (this.tracer) {
      const span = this.tracer.startSpan('voice_biomarker_analysis');
      span.setAttribute('session_id', data.sessionId);
      span.setAttribute('jitter', data.biomarkers?.jitter || 0);
      span.setAttribute('shimmer', data.biomarkers?.shimmer || 0);
      span.setAttribute('hnr', data.biomarkers?.hnr || 0);
      span.setAttribute('syllable_rate', data.biomarkers?.syllableRate || 0);
      span.setAttribute('updrs_score', data.updrsScore?.score || 0);

      // Set error status for high UPDRS scores
      if (data.updrsScore?.score >= 3) {
        span.setStatus({ code: SpanStatusCode.ERROR });
      }
      span.end();
    }

    console.log(`[ARIZE] Voice biomarkers - UPDRS: ${data.updrsScore?.score || 'N/A'}, Jitter: ${data.biomarkers?.jitter?.toFixed(2)}%`);
  }
